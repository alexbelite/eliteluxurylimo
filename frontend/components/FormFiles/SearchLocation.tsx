...
export default function SearchLocation() {
  ...
  const airportName = useSelector(
    (state: RootState) => state.map.results.airportName
  );
  const serviceDetail = useSelector(
    (state: RootState) => state.map.results.serviceDetail
  );
  const serviceHours = useSelector(
    (state: RootState) => state.map.results.hours
  );
  ...
  return (
    <>
      ...
      <div
        ref={ref}
        className="p-4 w-full  flex justify-between items-start gap-3 max-sm:flex-col"
      >
        {serviceDetail === "from" ? (
          <></>
        ) : (
          <div
            className={`${
              serviceDetail === "from" || serviceDetail === "hourly"
                ? "w-1/2"
                : "w-full"
            } max-sm:w-full relative`}
          >
            <Input
              isRequired
            ...
            />
            {status === "OK" && (
              <ul className="bg-white absolute z-10">{renderSuggestions()}</ul>
            )}
          </div>
        )}
        {serviceDetail === "to" ? (
          ...
          <div
            className={`${
              serviceDetail === "to" || serviceDetail === "hourly"
                ? "w-1/2"
                : "w-full"
          ...
          >
            ...
            />
            {statusB === "OK" && (
              <ul className="bg-white absolute z-10">{renderSuggestionsB()}</ul>
            )}
          </div>
        )}
      </div>
