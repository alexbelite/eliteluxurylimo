...
export default function PeopleForm() {
  ...
  const travellers = useSelector(
    (state: RootState) => state.map.results.travellers
  );
  const bags = useSelector((state: RootState) => state.map.results.bags);
  const res = useSelector((state: RootState) => state.map.results);
  const reduxVehicle = useSelector(
    (state: RootState) => state.map.results.selectedVehicle
  );
  const findAddress = airports.find(
    (airport) => airport.name === res.airportName
  );
  ...
  const [scrollBehavior, setScrollBehavior] =
    React.useState<ScrollBehavior>("normal");
  ...
  const airportAddress = useSelector(
    (state: RootState) => state.map.location.address
  );
  const airportAddressB = useSelector(
    (state: RootState) => state.map.locationB.address
  );
  ...
  return (
    <>
      <div className="flex flex-col gap-3 h-auto mb-10">
        <section className="flex gap-3 flex-wrap h-auto ">
          <div className="max-lg:items-start flex-1 flex flex-col gap-3">
            ...
            <ButtonGroup
              radius="none"
              size="md"
              className="flex justify-stretch items-stretch w-full"
            >
              <Button onClick={handleTravellersDecrement} isIconOnly>
                <MinusIcon />
              </Button>
              <Button
                className="font-semibold leading-7 text-lg"
                isDisabled
                isIconOnly
              >
                {travellers}
              </Button>
              <Button onClick={handleTravellersIncrement} isIconOnly>
                <PlusIcon />
              </Button>
            </ButtonGroup>
          </div>
          <div className="max-lg:items-start flex-1  flex flex-col gap-3">
            ...
            <ButtonGroup
              ...
              className="flex justify-stretch items-stretch w-full"
            >
              <Button onClick={handleBagsDecrement} isIconOnly>
                <MinusIcon />
              </Button>
              <Button
                className="font-semibold leading-7 text-lg"
              ...
              <Button onClick={handleBagsIncrement} isIconOnly>
                <PlusIcon />
              </Button>
            </ButtonGroup>
          </div>
        ...
        <Button
          onPress={CheckPricing}
          className="bg-black lg:w-full  text-white mr-auto px-8"
          radius="none"
        >
          Check Pricing
        </Button>
            ...
            <>
                ...
                {vehicleSelected === false ? (
                  vehicles.map((el, index) => (
                    <div
                      ...
                      key={index}
                      onClick={() => onVehicleSelected(el)}
                    >
                      <Card
                        isFooterBlurred
                        className="relative hover:drop-shadow-xl w-full h-[300px]"
                      >
                        <CldImage
                          fill
                          alt="alexlimo"
                          ...
                          <div className="flex flex-grow gap-2 items-center">
                            <div className="flex flex-col">
                              ...
                              <p className="text-tiny text-white">
                                Passengers: {el.passengers} - Bags: {el.bags}
                              </p>
                            </div>
                          </div>
                      ...
                      <div className="w-full">
                        <div className="w-full h-full">
                          <Image
                            ...
                            src={reduxVehicle?.image}
                            alt="alex limo"
                          />
                          <div>
                            {res?.serviceDetail === "from" && (
                              <p>Pick-up From {res?.airportName}</p>
                            )}
                            {res?.serviceDetail === "to" && (
                              <p>Drop-off to {res?.airportName}</p>
                            )}
                            {["from", "to"].includes(res?.serviceDetail) && (
                              <p>Airport address: {findAddress?.address}</p>
                            )}
                            {res?.serviceDetail === "from" && (
                              <p>Drop-off To {res?.dropoff}</p>
                            )}
                            {res?.serviceDetail === "to" && (
                              <p>Pick-up from {res?.pickup}</p>
                            )}
                            {(res?.serviceDetail === "point" ||
                              res?.serviceDetail === "hourly") && (
                              <p>Pick-up From {res?.pickup}</p>
                            )}
                            {(res?.serviceDetail === "point" ||
                              res?.serviceDetail === "hourly") && (
                      ...
                      <div className="w-full h-full">
                        ...
                        {requestSent ? (
                          ...
                          <form action={onSubmit}>
                            ...
                            <button
                              className="bg-white rounded-sm text-black px-3 py-2"
                              type="submit"
                            >
                              Request a Quote
                            </button>
              ...
              <ModalFooter>
                <Button
                  color="danger"
                  variant="light"
                  onPress={vehicleSelected === false ? onClose : onCanceled}
                >
                  Close
                </Button>
              </ModalFooter>
